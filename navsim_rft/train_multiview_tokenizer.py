...
def main():
    ...
    p.add_argument('--steps',type=int,required=True); p.add_argument('--save-every',type=int,default=500)
    ...
    for step in range(a.steps):
        item=ds[(step*world+rank)%len(ds)]; views=item['views'].to(device); cam=int((step+rank)%views.shape[0])
