# Derived from supplied prismatic/models/diffusion_transformer.py; see docs/navsim/DIFFERENCES.md
...
import numpy as np
from navsim_rft.mlp import Mlp
from navsim_rft.transformer_utils import CrossAttentionBlock, PerceiverResampler, MAPBlock
    ...
    def forward(self, x, mask = None):
        ...
        if self.attention_mode == 'xformers': # cause loss nan while using with amp
            ...
            if mask is not None:
                ...
                attn = attn.masked_fill(mask.to(attn.device) == 0, _MASKING_VALUE)
...
class DiT_SingleTokenAction(nn.Module):
    ...
    def __init__(
        ...
        self.proprio_embedder = nn.Linear(context_dim, hidden_size)

        self.context_adapter = nn.Linear(context_dim, hidden_size)
    ...
    def forward(self,
        ...
        t = self.t_embedder(timesteps.reshape(-1), use_bfp16=False).unsqueeze(1)
...
class DiT_SingleTokenAction_OneCtx(DiT_SingleTokenAction):
    ...
    def _prepare_context(self, context: torch.Tensor) -> torch.Tensor:
        ...
        if Dctx != self.context_adapter.in_features:
            raise ValueError(
    ...
    def forward(
        ...
        t_emb = self.t_embedder(timesteps.reshape(-1), use_bfp16=False).unsqueeze(1)            # (B, hidden)
