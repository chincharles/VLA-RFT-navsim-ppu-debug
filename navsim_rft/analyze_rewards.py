...
def main():
    ...
    with torch.no_grad():
        for i in range(min(a.limit,len(ds))):
            ...
            for j in range(n):rows.append(dict(token=d['token'],candidate=j,image_reward=float(ri[j]),
                log_image_reward=float(rl[j]),pdms=float(pdms[j]),trajectory=pose[j].tolist()))
