...
def main():
    ...
    env.update(VIRTUAL_ENV=str(root),PATH=str(root/'bin')+os.pathsep+env['PATH'],
        RFT_RUNTIME='ppu',PYTHONNOUSERSITE='1',PYTHONPATH=os.pathsep.join(map(str,[
            REPO/'configs/navsim/ppu_compat',REPO,REPO/'train/verl',
            REPO/'train/verl/vla-adapter/openvla-oft',Path(env['NAVSIM_ROOT'])])))
    env['PIP_CONSTRAINT']=str(constraints)
    report=out/'pip-plan.json'
    command=[str(py),'-m','pip','install','-r',str(REPO/'configs/navsim/requirements-ppu.txt'),'-c',str(constraints)]
    ...
    subprocess.run([str(py),str(REPO/'scripts/navsim/check_ppu_dependencies.py'),'--output',str(out/'ppu-imports.json')],env=env,check=True)
