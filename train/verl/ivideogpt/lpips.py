...
class LPIPS(nn.Module):
    ...
    def forward(self, input, target, weight=None):
        ...
        res = [spatial_average(lins[kk].model(diffs[kk]), keepdim=True, weight=weight) for kk in range(len(self.chns))]
        val = res[0]
...
def spatial_average(x, keepdim=True, weight=None):
    if weight is not None:
        resized_weight = F.interpolate(weight, x.shape[-2:])
        x = x * resized_weight
